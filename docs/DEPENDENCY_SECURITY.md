...
| `RUSTSEC-2024-0429` (`glib`) | `apivoy-desktop -> Tauri/Wry -> glib 0.18` | The advisory affects `VariantStrIter`; neither ApiVoy nor the resolved Tauri/Wry sources reference that API. Dependabot can resolve at most `glib 0.18.5` because `tauri-runtime` / `wry` still depend on `gtk 0.18`. The first patched crate is `0.20.0`, a breaking gtk-rs major. | Remove on a Tauri GTK stack upgrade or if the affected API becomes reachable. Do not force `glib 0.20` while the desktop shell remains on Tauri 2 WebKitGTK3. |
