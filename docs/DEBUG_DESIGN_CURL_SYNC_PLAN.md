...
调试与设计是同一份接口草稿的两个编辑视图；在任一视图中的结构修改应立即反映到另一视图，但只有“保存接口”后才更新已保存接口并影响关联用例。推荐模型是：

```text
Interface（接口资产）
├─ Interface Draft（调试 / 设计共享）
├─ Saved Definition（保存后的结构与约束）
└─ Cases（独立可保存的可执行值与局部结构差异）
```

- 调试和设计共享方法、路径、参数结构、Body Schema 与安全方案。
- 调试中的参数值、Secret Ref、脚本、断言、超时、代理和 TLS 属于当前默认调试数据或具体用例。
- 用例可以独立新增、删除参数并保存；结构不一致不是保存阻断条件。
- 用例与接口不一致时提供“同步接口”（以接口为准修改当前用例）和“更新接口”（以用例差异修改接口草稿）。
- 保存接口后才把结构变化应用到关联用例；不会静默删除不兼容的用例值。
...
|---|---|
| 新建调试 | 只建临时草稿 |
| 首次保存调试 | 保存统一接口结构和默认调试数据 |
| 调试或设计修改结构 | 两个视图实时共享草稿，保存接口后影响用例 |
| 新建设计 | 创建统一接口草稿，调试视图立即可用 |
| 用例新增或删除参数 | 允许保存，并显示参数不一致 |
| 用例选择“同步接口” | 以接口结构调整当前用例，不影响其他用例 |
| 用例选择“更新接口” | 将用例差异应用到接口草稿，保存接口后影响其他用例 |
| 首页导入 cURL | 新建接口及默认调试数据 |
| 资源树导入 cURL | 在当前项目/集合新建接口及默认调试数据 |
| 接口内导入 | 不提供通用入口；定义导入保留在设计页 |
