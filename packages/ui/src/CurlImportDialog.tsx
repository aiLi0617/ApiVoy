...
export function CurlImportDialog({ open, onClose, onCreate }: CurlImportDialogProps) {
  const titleId = useId(), inputRef = useRef<HTMLTextAreaElement>(null);
  ...
  if (!open) return null;
  return createPortal(<ModalFrame open={open} onClose={onClose} className="curl-import-dialog" ariaLabelledBy={titleId} initialFocusRef={inputRef} as="form" onSubmit={(event) => { event.preventDefault(); if (!result.request || busy) return; setBusy(true); Promise.resolve(onCreate(result.request)).then(() => setSource("")).finally(() => setBusy(false)); }}>
    <header className="settings-dialog-header"><div><h2 id={titleId}>{C.title}</h2><p>{C.intro}</p></div><IconButton label={C.close} icon="close" onClick={onClose} /></header>
    ...
    <footer className="settings-dialog-footer"><span>{C.hint}</span><Button variant="secondary" disabled={busy} onClick={onClose}>{C.cancel}</Button><Button type="submit" variant="primary" icon="download" disabled={!result.request || busy}>{busy ? "\u521b\u5efa\u4e2d\u2026" : C.submit}</Button></footer>
