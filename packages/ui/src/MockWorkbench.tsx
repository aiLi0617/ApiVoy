...
export function MockWorkbench({ baseUrl, onList, onCreate, onDelete }: MockWorkbenchProps) {
  ...
  return <section style={styles.root}>
    ...
    <div style={styles.form}>
      <input aria-label="Mock 规则名称" value={name} onChange={(event) => setName(event.target.value)} placeholder="规则名称" />
      ...
      <input aria-label="Mock 路径" value={path} onChange={(event) => setPath(event.target.value)} placeholder="/path" />
...
const styles: Record<string, CSSProperties> = {
  ...
  state: { display: "flex", alignItems: "center", gap: 8, marginTop: 13, padding: 12, border: "1px solid var(--apivoy-border)", borderRadius: 8, color: "var(--apivoy-muted)", fontSize: 11 },
