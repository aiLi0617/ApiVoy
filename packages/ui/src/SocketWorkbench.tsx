...
function SocketWorkbench({ protocol, onSend, onCancel, onSave, externalRequest, tcpSessionUrl, onTitleChange }: SocketWorkbenchProps & { protocol: "tcp" | "udp" }) {
  ...
  useEffect(() => { const apply = (value: SocketWorkbenchRequest) => { if (value.protocol !== protocol) return; if (value.id) requestIdRef.current = value.id; setName(value.name ?? ""); setTarget(value.target); setData(value.sourceData ?? value.data); setEncoding(value.format ?? value.encoding); setFraming(value.framing ?? "none"); setDelimiter(value.delimiter ?? "\\n"); setFixedLength(value.fixedLength ?? 1); setSendCount(value.sendCount); setIntervalMs(value.intervalMs); setTimeoutMs(value.timeoutMs); setTls(value.tls); setServerName(value.serverName ?? ""); setCaCertRef(value.caCertRef ?? ""); }; if (externalRequest) apply(externalRequest); else { const draft = readWorkbenchDraft<SocketWorkbenchRequest>(protocol) ?? readWorkbenchDraft<SocketWorkbenchRequest>("socket"); if (draft) apply(draft); } const listener = (event: Event) => { const envelope = (event as CustomEvent).detail; const payload = envelope?.payload; if (payload?.type !== protocol) return; apply({ id: envelope.id, protocol, name: envelope.name, target: envelope.target, data: payload.data, encoding: payload.encoding, format: envelope.metadata?.socketPayloadFormat ?? envelope.metadata?.tcpPayloadFormat, sourceData: envelope.metadata?.socketSourceData ?? envelope.metadata?.tcpSourceData, framing: protocol === "tcp" ? payload.framing : null, delimiter: protocol === "tcp" ? payload.delimiter : null, fixedLength: protocol === "tcp" ? payload.fixedLength : null, sendCount: payload.sendCount, intervalMs: payload.intervalMs, timeoutMs: envelope.timeoutMs, tls: protocol === "tcp" && payload.tls, serverName: protocol === "tcp" ? payload.serverName : null, caCertRef: protocol === "tcp" ? payload.caCertRef : null }); }; const pending = consumeHydrate(protocol); if (pending) listener(new CustomEvent("apivoy-open-request", { detail: pending.envelope }) as Event); const onHydrate = (event: Event) => { const d = (event as CustomEvent).detail; if (d?.workbenchId !== protocol) return; listener(new CustomEvent("apivoy-open-request", { detail: d.envelope }) as Event); }; window.addEventListener("apivoy-open-request", listener); window.addEventListener("apivoy-hydrate-request", onHydrate); return () => { window.removeEventListener("apivoy-open-request", listener); window.removeEventListener("apivoy-hydrate-request", onHydrate); }; }, [externalRequest, protocol]);
  ...
  if (protocol === "tcp") {
    ...
    return <div className="websocket-workbench-layout tcp-workbench-layout"><SplitPane id="tcp-workbench" direction="vertical" minPrimary={304} minSecondary={160} primaryLabel="TCP 请求配置" secondaryLabel="消息检查器" secondaryActions={connectionStatus} primary={<section className={`websocket-workbench websocket-request-pane tab-${requestTab}`}>
      ...
      <div className="websocket-request-tabs" role="tablist" aria-label="TCP 请求配置"><button type="button" role="tab" aria-selected={requestTab === "message"} className={requestTab === "message" ? "is-active" : ""} onClick={() => setRequestTab("message")}>Message</button><button type="button" role="tab" aria-selected={requestTab === "settings"} className={requestTab === "settings" ? "is-active" : ""} onClick={() => setRequestTab("settings")}>设置</button></div>
      ...
      {!hasConnected ? <div className="websocket-response-waiting"><strong>等待连接</strong><span>建立 TCP 连接后，发送和接收的消息会显示在这里。</span></div> : <><div className="websocket-response-tabs" role="tablist" aria-label="TCP 响应"><button type="button" role="tab" aria-selected="true" className="is-active">Messages <span>{frames.filter((frame) => frame.direction !== "•").length}</span></button></div><div className="websocket-frame-list"><div className={`websocket-message-browser${selectedFrame ? " has-detail" : ""}`}><section className="websocket-message-summary"><header className="websocket-message-toolbar"><input type="search" aria-label="搜索 TCP 消息" value={frameQuery} onChange={(event) => setFrameQuery(event.target.value)} placeholder="搜索消息"/><select aria-label="筛选 TCP 消息" value={frameFilter} onChange={(event) => setFrameFilter(event.target.value as typeof frameFilter)}><option value="all">全部消息</option><option value="incoming">仅接收</option><option value="outgoing">仅发送</option></select><button type="button" className="websocket-clear-messages" aria-label="清空消息" title="清空消息" onClick={() => { setFrames([]); setSelectedFrameId(null); }}><Icon name="archive"/></button></header><div className="websocket-message-list">{visibleFrames.map((frame) => <button type="button" key={frame.id} className={`websocket-frame websocket-frame-${frame.direction === "←" ? "incoming" : frame.direction === "→" ? "outgoing" : "status"}${selectedFrameId === frame.id ? " is-selected" : ""}`} aria-disabled={frame.direction === "•"} onClick={() => frame.direction !== "•" && setSelectedFrameId(frame.id)}><b>{frame.direction === "•" ? "✓" : frame.direction}</b><code>{frame.payload.replace(/\s+/g, " ").trim() || frame.type}</code><time>{frame.time}</time></button>)}</div></section>{selectedFrame && <section className="websocket-frame-detail"><header><div className="websocket-frame-detail-options"><select aria-label="消息展示格式" value={detailFormat} onChange={(event) => setDetailFormat(event.target.value as typeof detailFormat)}><option value="pretty">Pretty</option><option value="hexdump">Hexdump</option><option value="raw">Raw</option></select><select aria-label="消息内容类型" value={responseFormat} onChange={(event) => setResponseFormat(event.target.value as TcpPayloadFormat | "auto")}><option value="auto">Auto</option><option value="text">Text</option><option value="json">JSON</option><option value="xml">XML</option><option value="hex">HEX</option><option value="base64">Base64</option><option value="msgpack">MessagePack</option></select><button type="button" className={detailWordWrap ? "is-active" : ""} aria-label="切换自动换行" title="自动换行" onClick={() => setDetailWordWrap((current) => !current)}><Icon name="wrap"/></button></div><div className="websocket-frame-detail-actions"><button type="button" aria-label="下载消息" title="下载" onClick={downloadSelectedFrame}><Icon name="download"/></button><button type="button" aria-label="复制消息" title="复制" onClick={() => void navigator.clipboard.writeText(selectedFramePayload)}><Icon name="copy"/></button><button type="button" aria-label="搜索消息内容" title="搜索" onClick={() => detailEditorRef.current?.openFind()}><Icon name="search"/></button><button type="button" aria-label="关闭消息详情" title="关闭" onClick={() => setSelectedFrameId(null)}><Icon name="close"/></button></div></header><CodeEditor ref={detailEditorRef} value={selectedFramePayload} onChange={() => {}} language={detailFormat === "pretty" ? tcpPayloadLanguage(selectedContentFormat) : "plaintext"} height="100%" wordWrap={detailWordWrap} readOnly bare/></section>}</div></div></>}
    </section>} /></div>;
  }
  ...
  return <div className="websocket-workbench-layout udp-workbench-layout"><SplitPane id="udp-workbench" direction="vertical" minPrimary={304} minSecondary={160} primaryLabel="UDP" secondaryLabel="数据报检查器" secondaryActions={udpStatus} primary={<section className={"websocket-workbench websocket-request-pane tab-" + requestTab}>
    ...
    <div className="websocket-request-tabs" role="tablist" aria-label="UDP 请求配置"><button type="button" role="tab" aria-selected={requestTab === "message"} className={requestTab === "message" ? "is-active" : ""} onClick={() => setRequestTab("message")}>Message</button><button type="button" role="tab" aria-selected={requestTab === "settings"} className={requestTab === "settings" ? "is-active" : ""} onClick={() => setRequestTab("settings")}>设置</button></div>
    ...
    {!hasConnected ? <div className="websocket-response-waiting"><strong>等待发送</strong><span>发送 UDP 数据报后，收发记录与超时状态会显示在这里。</span></div> : <><div className="websocket-response-tabs" role="tablist" aria-label="UDP 数据报"><button type="button" role="tab" aria-selected="true" className="is-active">Datagrams <span>{frames.filter((frame) => frame.direction !== "•").length}</span></button></div><div className="websocket-frame-list"><div className={"websocket-message-browser" + (udpSelectedFrame ? " has-detail" : "")}><section className="websocket-message-summary"><header className="websocket-message-toolbar"><input type="search" aria-label="搜索 UDP 数据报" value={frameQuery} onChange={(event) => setFrameQuery(event.target.value)} placeholder="搜索数据报"/><select aria-label="筛选 UDP 数据报" value={frameFilter} onChange={(event) => setFrameFilter(event.target.value as typeof frameFilter)}><option value="all">全部数据报</option><option value="incoming">仅接收</option><option value="outgoing">仅发送</option></select><button type="button" className="websocket-clear-messages" aria-label="清空数据报" title="清空" onClick={() => { setFrames([]); setSelectedFrameId(null); }}><Icon name="archive"/></button></header><div className="websocket-message-list">{udpVisibleFrames.map((frame) => <button type="button" key={frame.id} className={"websocket-frame websocket-frame-" + (frame.direction === "←" ? "incoming" : frame.direction === "→" ? "outgoing" : "status") + (selectedFrameId === frame.id ? " is-selected" : "")} aria-disabled={frame.direction === "•"} onClick={() => frame.direction !== "•" && setSelectedFrameId(frame.id)}><b>{frame.direction === "•" ? "•" : frame.direction}</b><code>{frame.payload.replace(/\s+/g, " ").trim() || frame.type}</code><span className="udp-frame-meta">{frame.type}</span><time>{frame.time}</time></button>)}</div></section>{udpSelectedFrame && <section className="websocket-frame-detail"><header><div className="websocket-frame-detail-options"><select aria-label="数据报展示格式" value={detailFormat} onChange={(event) => setDetailFormat(event.target.value as typeof detailFormat)}><option value="pretty">Pretty</option><option value="hexdump">Hexdump</option><option value="raw">Raw</option></select><select aria-label="数据报内容类型" value={responseFormat} onChange={(event) => setResponseFormat(event.target.value as TcpPayloadFormat | "auto")}><option value="auto">Auto</option><option value="text">Text</option><option value="json">JSON</option><option value="xml">XML</option><option value="hex">HEX</option><option value="base64">Base64</option><option value="msgpack">MessagePack</option></select><button type="button" className={detailWordWrap ? "is-active" : ""} aria-label="切换自动换行" title="自动换行" onClick={() => setDetailWordWrap((current) => !current)}><Icon name="wrap"/></button></div><div className="websocket-frame-detail-actions"><button type="button" aria-label="下载数据报" title="下载" onClick={downloadUdpFrame}><Icon name="download"/></button><button type="button" aria-label="复制数据报" title="复制" onClick={() => void navigator.clipboard.writeText(udpSelectedPayload)}><Icon name="copy"/></button><button type="button" aria-label="搜索数据报内容" title="搜索" onClick={() => detailEditorRef.current?.openFind()}><Icon name="search"/></button><button type="button" aria-label="关闭数据报详情" title="关闭" onClick={() => setSelectedFrameId(null)}><Icon name="close"/></button></div></header><CodeEditor ref={detailEditorRef} value={udpSelectedPayload} onChange={() => {}} language={detailFormat === "pretty" ? tcpPayloadLanguage(udpSelectedContentFormat) : "plaintext"} height="100%" wordWrap={detailWordWrap} readOnly bare/></section>}</div></div></>}
  </section>} /></div>;
}
