...
export function WorkbenchFrame({ title, description, badge, toolbar, children, status, busy = false, hideHeader = false }: WorkbenchFrameProps) { const titleId = `workbench-title-${title.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}`; return <section className="workbench-frame" aria-label={hideHeader ? title : undefined} aria-labelledby={hideHeader ? undefined : titleId} aria-busy={busy || undefined}>{hideHeader ? null : <header className="workbench-frame-header"><div><div className="workbench-title-line"><h1 id={titleId}>{title}</h1>{badge}</div>{description ? <p>{description}</p> : null}</div>{toolbar ? <div className="workbench-toolbar">{toolbar}</div> : null}</header>}<div className="workbench-frame-body">{children}</div>{status || busy ? <footer className="workbench-status" role="status" aria-live="polite">{status ?? "处理中…"}</footer> : null}</section>; }
