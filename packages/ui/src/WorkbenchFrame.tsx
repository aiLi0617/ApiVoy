...
export function SplitPane({ id, primary, secondary, secondaryActions, direction = "vertical", minPrimary = 280, minSecondary = 240, primaryLabel = "请求", secondaryLabel = "响应" }: SplitPaneProps) {
  ...
  return <div className="split-pane-shell">
    <div ref={rootRef} className={`split-pane split-${actualDirection}${collapsedPanel ? ` split-${collapsedPanel}-collapsed` : ""}`} style={{ "--split-ratio": `${ratio * 100}%`, "--split-min-primary": `${minPrimary}px`, "--split-min-secondary": `${minSecondary}px` } as React.CSSProperties}>
      ...
      <section className="split-panel split-secondary-panel" aria-label={secondaryLabel}><div className="split-secondary-horizontal-header"><button type="button" className="split-secondary-header-toggle" aria-label={collapsedPanel === "secondary" ? `展开${secondaryLabel}` : `收起${secondaryLabel}`} aria-expanded={collapsedPanel !== "secondary"} onClick={toggleSecondary}><span className="split-response-toggle-icon" aria-hidden="true"/><span>{secondaryLabel === "响应检查器" ? "返回响应" : secondaryLabel}</span></button>{secondaryActions ? <div className="split-secondary-header-actions">{secondaryActions}</div> : null}</div><div className="split-secondary-content">{secondary}</div><div className="split-layout-flyout"><div className="split-layout-popover" role="group" aria-label="选择分栏布局">{layouts.map((layout) => <button key={layout.id} type="button" className={actualDirection === layout.id ? "is-active" : ""} aria-pressed={actualDirection === layout.id} title={layout.title} onClick={() => setDirection(layout.id)}><SplitLayoutIcon direction={layout.id}/><span>{layout.label}分屏</span></button>)}</div><button type="button" className="split-layout-trigger" aria-label="切换分栏布局" title="切换分栏布局"><SplitLayoutIcon direction={actualDirection}/></button></div></section>
    </div>
  </div>;
}

export interface WorkbenchFrameProps { title: string; description?: string; badge?: ReactNode; toolbar?: ReactNode; children: ReactNode; status?: ReactNode; busy?: boolean }
export function WorkbenchFrame({ title, description, badge, toolbar, children, status, busy = false }: WorkbenchFrameProps) { const titleId = `workbench-title-${title.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}`; return <section className="workbench-frame" aria-labelledby={titleId} aria-busy={busy || undefined}><header className="workbench-frame-header"><div><div className="workbench-title-line"><h1 id={titleId}>{title}</h1>{badge}</div>{description ? <p>{description}</p> : null}</div>{toolbar ? <div className="workbench-toolbar">{toolbar}</div> : null}</header><div className="workbench-frame-body">{children}</div>{status || busy ? <footer className="workbench-status" role="status" aria-live="polite">{status ?? "处理中…"}</footer> : null}</section>; }
