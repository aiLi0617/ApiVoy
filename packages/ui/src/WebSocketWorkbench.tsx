...
export function WebSocketWorkbench({ onConnect, onCancel, onSave, externalRequest }: WebSocketWorkbenchProps) {
  ...
  return <section style={styles.root}>
    ...
    <textarea aria-label="WebSocket 消息" style={styles.message} value={message} onChange={(event) => setMessage(event.target.value)} placeholder={encoding === "binary" ? "Base64 binary payload" : "Text frame payload"}/><ProtocolCodeGenerator input={{ protocol: "websocket", request: request() }} /><div style={styles.timelineTitle}>帧时间线 / 响应</div><pre style={styles.output}>{output}</pre>
  </section>;
}
