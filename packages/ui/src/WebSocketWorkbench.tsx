...
import { KeyValueRows, createQueryRow, queryRowsFromUrl, urlWithQueryRows, type HeaderRow } from "./KeyValueEditor";
...
export function WebSocketWorkbench({ onSave, externalRequest, onTitleChange }: WebSocketWorkbenchProps) {
  ...
  function applyRequest(value: WebSocketWorkbenchRequest) { setName(value.name ?? ""); setUrl(value.url); setQueryRows(queryRowsFromUrl(value.url)); setHeaderRows(rowsFromPairs(value.headers.filter(([header]) => header.toLowerCase() !== "cookie"))); setCookieRows(cookieRowsFromHeaders(value.headers)); setSubprotocols(value.subprotocols.join(", ")); const first = value.messages[0]; setMessageFormat(first?.encoding === "binary" ? "binary" : "text"); setMessage(first?.data ?? ""); setReceiveLimit(value.receiveLimit); setTimeoutMs(value.timeoutMs); setReconnectMax(value.reconnectMax); setReconnectDelayMs(value.reconnectDelayMs); }
  useEffect(() => { if (externalRequest) applyRequest(externalRequest); else { const draft = readWorkbenchDraft<WebSocketWorkbenchRequest>("websocket"); if (draft) applyRequest(draft); } }, [externalRequest]);
  useWorkbenchHydration("websocket", (detail) => { const envelope = detail as { name?: string; target?: string; timeoutMs?: number; payload?: Partial<WebSocketWorkbenchRequest> & { type?: string } }; const payload = envelope.payload; if (payload?.type !== "websocket") return; applyRequest({ name: envelope.name ?? "", url: envelope.target ?? "", headers: payload.headers ?? [], subprotocols: payload.subprotocols ?? [], messages: payload.messages ?? [], receiveLimit: payload.receiveLimit ?? 1, timeoutMs: envelope.timeoutMs ?? 30_000, reconnectMax: payload.reconnectMax ?? 0, reconnectDelayMs: payload.reconnectDelayMs ?? 1_000 }); });
  ...
  return <div className="websocket-workbench-layout"><SplitPane id="websocket-workbench" direction="vertical" minPrimary={304} minSecondary={160} primaryLabel="WebSocket 请求配置" secondaryLabel="响应检查器" secondaryActions={connectionStatus} primary={<section className={`websocket-workbench websocket-request-pane tab-${requestTab}`}>
    ...
    <RovingTabList className="websocket-request-tabs" ariaLabel="WebSocket 请求配置">{([['message','Message'],['params','Params'],['headers','Headers'],['cookies','Cookies'],['settings','设置']] as const).map(([id,label]) => <button key={id} type="button" role="tab" aria-selected={requestTab === id} tabIndex={requestTab === id ? 0 : -1} className={requestTab === id ? "is-active" : ""} onClick={() => setRequestTab(id)}>{label}</button>)}</RovingTabList>
    ...
    {requestTab === "settings" && <section className="websocket-settings-panel"><div className="websocket-settings-fields websocket-settings-fields-single"><label><span>子协议</span><input value={subprotocols} onChange={(event) => setSubprotocols(event.target.value)} placeholder="graphql-transport-ws"/></label></div></section>}
      ...
      <RovingTabList className="websocket-response-tabs" ariaLabel="WebSocket 响应">
      ...
      <div className="websocket-frame-list" role="tabpanel">
        {responseTab === "messages" && <MessageInspector ref={messageBrowserRef} hasDetail={Boolean(selectedFrame)} stacked={responseDetailsStacked} style={{ "--websocket-message-list-ratio": `${messageListRatio}%` } as CSSProperties}>
          <MessageSummary><MessageToolbar><input type="search" aria-label="搜索 WebSocket 消息" value={frameQuery} onChange={(event) => setFrameQuery(event.target.value)} placeholder="搜索消息"/><select aria-label="筛选 WebSocket 消息" value={frameFilter} onChange={(event) => setFrameFilter(event.target.value as typeof frameFilter)}><option value="all">全部消息</option><option value="incoming">仅接收</option><option value="outgoing">仅发送</option></select><button type="button" className="websocket-clear-messages" aria-label="清空消息" title="清空消息" onClick={() => { setFrames([]); setSelectedFrameId(null); }}><Icon name="archive"/></button></MessageToolbar><div className="websocket-message-list">{visibleFrames.map((frame) => <button type="button" key={frame.id} className={`websocket-frame websocket-frame-${frame.direction === "←" ? "incoming" : frame.direction === "→" ? "outgoing" : "status"}${selectedFrameId === frame.id ? " is-selected" : ""}`} aria-disabled={frame.direction === "•"} onClick={() => { if (frame.direction !== "•") setSelectedFrameId(frame.id); }}><b>{frame.direction === "•" ? "✓" : frame.direction}</b><code>{frame.payload ? displayFramePayload(frame.payload).replace(/\s+/g, " ").trim() : frame.type}</code><time>{frame.time}</time></button>)}</div></MessageSummary>
          {selectedFrame && <><div className="websocket-detail-resizer" role="separator" tabIndex={0} aria-label="调整消息列表和详情区域大小" aria-orientation={responseDetailsStacked ? "horizontal" : "vertical"} aria-valuemin={22} aria-valuemax={78} aria-valuenow={Math.round(messageListRatio)} onPointerDown={resizeMessagePanels} onKeyDown={resizeMessagePanelsWithKeyboard} onDoubleClick={() => setMessageListRatio(46)}/><section className="websocket-frame-detail"><header><div className="websocket-frame-detail-options"><select aria-label="消息展示格式" value={detailFormat} onChange={(event) => setDetailFormat(event.target.value as typeof detailFormat)}><option value="pretty">Pretty</option><option value="hexdump">Hexdump</option><option value="raw">Raw</option></select><select aria-label="消息内容类型" value={detailContentType} onChange={(event) => setDetailContentType(event.target.value as typeof detailContentType)}><option value="auto">Auto</option><option value="json">JSON</option><option value="xml">XML</option><option value="html">HTML</option><option value="javascript">JavaScript</option><option value="text">Text</option></select><select aria-label="消息字符编码" value={detailCharset} onChange={(event) => setDetailCharset(event.target.value as typeof detailCharset)}><option value="auto">Auto</option><option value="utf-8">UTF-8</option><option value="gb18030">GBK / GB18030</option><option value="utf-16le">UTF-16 LE</option><option value="utf-16be">UTF-16 BE</option><option value="windows-1252">Windows-1252</option><option value="iso-8859-1">ISO-8859-1</option></select><button type="button" className={detailWordWrap ? "is-active" : ""} aria-label="切换自动换行" title="自动换行" onClick={() => setDetailWordWrap((current) => !current)}><Icon name="wrap"/></button></div><MessageDetailActions actions={[{ id: "download", label: "下载消息", icon: "download", className: "websocket-detail-direct-action", onSelect: downloadSelectedFrame }, { id: "copy", label: "复制消息", icon: "copy", className: "websocket-detail-direct-action", onSelect: () => void copySelectedFrame() }, { id: "search", label: "搜索消息内容", icon: "search", className: "websocket-detail-direct-action", onSelect: () => detailEditorRef.current?.openFind() }]} onClose={() => setSelectedFrameId(null)}/></header><CodeEditor ref={detailEditorRef} value={selectedFramePayload} onChange={() => {}} language={selectedFrameLanguage} height="100%" wordWrap={detailWordWrap} readOnly bare/></section></>}
        </MessageInspector>}
