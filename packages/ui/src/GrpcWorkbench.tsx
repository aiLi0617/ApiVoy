...
import { KeyValueRows, createQueryRow, type HeaderRow, type HttpRunResult, type HttpSendHooks } from "./HttpWorkbench";
...
export function GrpcWorkbench({ onSend, onCancel, onSave, externalRequest, onTitleChange }: GrpcWorkbenchProps) {
  ...
  useEffect(() => {
    const apply = (value: GrpcWorkbenchRequest) => { setTarget(value.target); setService(value.service); setMethod(value.method); setMode(value.mode); const json = value.messageJson; setMessageMode(json != null ? "json" : "base64"); setMessage(json ?? value.messageBase64); setDescriptorSetBase64(value.descriptorSetBase64 ?? ""); setDescriptorName(value.descriptorSetBase64 ? "DescriptorSet" : ""); setMetadata(metadataRows(value.metadata)); setTimeoutMs(value.timeoutMs); };
    if (externalRequest) apply(externalRequest); else { const draft = readWorkbenchDraft<GrpcWorkbenchRequest>("grpc"); if (draft) apply(draft); }
    const listener = (event: Event) => { const envelope = (event as CustomEvent).detail; const payload = envelope?.payload; if (payload?.type === "grpc") apply({ name: envelope.name, target: envelope.target, service: payload.service, method: payload.method, messageBase64: payload.messageBase64, messageJson: payload.messageJson, descriptorSetBase64: payload.descriptorSetBase64, mode: payload.mode, metadata: payload.metadata, timeoutMs: envelope.timeoutMs }); };
    const pending = consumeHydrate("grpc"); if (pending) listener(new CustomEvent("apivoy-open-request", { detail: pending.envelope }) as Event);
    const onHydrate = (event: Event) => { const detail = (event as CustomEvent).detail; if (detail?.workbenchId === "grpc") listener(new CustomEvent("apivoy-open-request", { detail: detail.envelope }) as Event); };
    window.addEventListener("apivoy-open-request", listener); window.addEventListener("apivoy-hydrate-request", onHydrate);
    return () => { window.removeEventListener("apivoy-open-request", listener); window.removeEventListener("apivoy-hydrate-request", onHydrate); };
  }, [externalRequest]);
  ...
  return <div className="grpc-workbench-layout">
    ...
    <div className="grpc-workbench-split"><SplitPane id="grpc-workbench" direction="vertical" minPrimary={260} minSecondary={180} primaryLabel="gRPC 请求配置" secondaryLabel="响应检查器" secondaryActions={status} primary={<section className={`grpc-request-pane tab-${requestTab}`}>
      ...
      <div className="grpc-request-tabs" role="tablist" aria-label="gRPC 请求配置">{tabs.map(([id, label]) => <button key={id} type="button" role="tab" aria-selected={requestTab === id} className={requestTab === id ? "is-active" : ""} onClick={() => setRequestTab(id)}>{label}</button>)}</div>
      ...
      <div className="websocket-response-tabs" role="tablist" aria-label="gRPC 响应">{([['messages', `Messages ${responseMessages.length}`], ['headers', `Headers ${responseMeta?.headers.length ?? 0}`], ['request', '实际请求']] as const).map(([id, label]) => <button key={id} type="button" role="tab" aria-selected={responseTab === id} className={responseTab === id ? "is-active" : ""} onClick={() => setResponseTab(id)}>{label}</button>)}</div>
      <div className="websocket-frame-list" role="tabpanel">
        {responseTab === "messages" && <div className={`websocket-message-browser${selectedMessage ? " has-detail" : ""}${responseDetailsStacked ? " is-stacked" : ""}`}><section className="websocket-message-summary"><header className="websocket-message-toolbar"><input type="search" aria-label="搜索 gRPC 响应消息" value={messageQuery} onChange={(event) => setMessageQuery(event.target.value)} placeholder="搜索响应消息"/><span className="grpc-message-count">{responseMessages.length} 条消息</span><button type="button" className="websocket-clear-messages" aria-label="清空 gRPC 响应消息" title="清空消息" onClick={() => { setResponseMessages([]); setSelectedMessageId(null); }}><Icon name="archive"/></button></header><div className="websocket-message-list">{visibleMessages.map((item) => <button type="button" key={item.id} className={`websocket-frame websocket-frame-incoming${selectedMessageId === item.id ? " is-selected" : ""}`} onClick={() => setSelectedMessageId(item.id)}><b>←</b><code>{item.preview.replace(/\s+/g, " ").trim() || `Message #${item.index}`}</code><span className="grpc-frame-size">{item.size} B</span><time>{item.time}</time></button>)}{!responseMessages.length && <div className="grpc-response-empty">{responseNotice}</div>}</div></section>{selectedMessage && <section className="websocket-frame-detail"><header><div className="websocket-frame-detail-options"><select aria-label="gRPC 消息展示格式" value={detailFormat} onChange={(event) => setDetailFormat(event.target.value as typeof detailFormat)}><option value="pretty">Pretty</option><option value="raw">Raw</option><option value="hexdump">Hexdump</option><option value="base64">Base64</option></select><button type="button" className={detailWordWrap ? "is-active" : ""} aria-label="切换自动换行" title="自动换行" onClick={() => setDetailWordWrap((current) => !current)}><Icon name="wrap"/></button></div><div className="websocket-frame-detail-actions"><button type="button" aria-label="下载 gRPC 消息" title="下载" onClick={downloadSelectedMessage}><Icon name="download"/></button><button type="button" aria-label="复制 gRPC 消息" title="复制" onClick={() => void navigator.clipboard.writeText(selectedMessagePayload)}><Icon name="copy"/></button><button type="button" aria-label="搜索 gRPC 消息内容" title="搜索" onClick={() => detailEditorRef.current?.openFind()}><Icon name="search"/></button><button type="button" aria-label="关闭 gRPC 消息详情" title="关闭" onClick={() => setSelectedMessageId(null)}><Icon name="close"/></button></div></header><CodeEditor ref={detailEditorRef} value={selectedMessagePayload} onChange={() => {}} language={selectedMessageLanguage} height="100%" wordWrap={detailWordWrap} readOnly bare/></section>}</div>}
